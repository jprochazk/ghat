...
export interface GithubEventPayloadTypeMap {
 ...
 */
export interface GitHubContextData<Triggers> {
  /**
...
export interface GitHubContext<Triggers> {
...
export interface MatrixContext<Matrix> {
  matrix: Matrix,
}
...
export type MatrixValueForKey<T, K extends string> =
...
export type MatrixType<T> = { [K in MatrixTypeKeys<T>]: MatrixValueForKey<T, K> };


export interface InputsContext<Triggers> {
...
export type ValueOrFactory<Result, Context> =
...
export type Allowed = LowerAlpha | Digit | "_";
...
export type NormalizeId<S extends string> =
...
export type WorkflowRunNameContext<Triggers> = GitHubContext<Triggers> & InputsContext<Triggers> & VarsContext;
export type WorkflowConcurrencyContext<Triggers> = GitHubContext<Triggers> & InputsContext<Triggers> & VarsContext;
export type WorkflowEnvContext<Triggers> = GitHubContext<Triggers> & SecretsContext & InputsContext<Triggers> & VarsContext;

export interface Workflow<Triggers> {
...
export interface FilteredTrigger {
  /**
...
export type BaseInput<Required extends boolean> = {
  required?: Required;
  description?: string;
};

export type BooleanInput<Required extends boolean> = BaseInput<Required> & { type: "boolean"; default?: boolean };
export type StringInput<Required extends boolean> = BaseInput<Required> & { type: "string"; default?: string };
export type NumberInput<Required extends boolean> = BaseInput<Required> & { type: "number"; default?: number };
export type ChoiceInput<O extends readonly [string, ...string[]], Required extends boolean> = BaseInput<Required> & {
  type: "choice";
  options: O;
  default?: O[number];
};

export interface Trigger<Inputs> {
...
export interface JobStrategy<Matrix> {
  matrix: Matrix,
  fail_fast?: boolean,
  max_parallel?: number,
}
...
export interface StepRef<Outputs = {}> {
  outputs: Outputs;
}

export type JobStrategyContext<Triggers, Needs> = GitHubContext<Triggers> & NeedsContext<Needs> & VarsContext & InputsContext<Triggers>;

export type JobIfContext<Triggers, Needs> = GitHubContext<Triggers> & NeedsContext<Needs> & VarsContext & InputsContext<Triggers>;

export type JobRunsOnContext<Triggers, Needs, Matrix> = GitHubContext<Triggers> & NeedsContext<Needs> & StrategyContext & MatrixContext<Matrix> & VarsContext & InputsContext<Triggers>;

export type JobEnvContext<Triggers, Needs, Matrix> = JobRunsOnContext<Triggers, Needs, Matrix> & SecretsContext;

// TODO: steps can reference other steps
export type StepsContext<Triggers, Needs, Matrix> =
...
export interface JobRef<Id extends string, Outputs> {
  id: Id;
}

/** Maps a tuple of JobRefs into a `{ needs: { <id>: { outputs: <Outputs> } } }` context. */
export type NeedsContext<T> = T extends JobRef<string, unknown>[]
  ? {
    needs: {
...
export declare function triggers<T extends Trigger<Inputs>, Inputs extends Record<string, any>>(trigger: T): T;

export declare function input<const Req extends boolean = false>(type: "boolean", options?: Omit<BooleanInput<Req>, "type">): BooleanInput<Req>;
export declare function input<const Req extends boolean = false>(type: "string", options?: Omit<StringInput<Req>, "type">): StringInput<Req>;
export declare function input<const Req extends boolean = false>(type: "number", options?: Omit<NumberInput<Req>, "type">): NumberInput<Req>;
...
export type MatrixInput = { include?: Record<string, unknown>[] } & Record<string, unknown[]>;

export declare function matrix<const T extends MatrixInput>(def: T): MatrixType<T>;

export interface WorkflowJobsContext<Triggers> extends GitHubContext<Triggers>, InputsContext<Triggers>, VarsContext {
...
export declare function workflow<const Triggers>(
  name: string,
...
export declare function run(script: string, options?: RunOptions): StepRef;

// ---- Step options ----

export interface UsesOptions<Inputs> extends StepOptions {
  with?: Inputs;
}

export interface UsesOptionsRequired<Inputs> extends StepOptions {
  with: Inputs;
}
 ...
 */
export declare function uses(action: never): never;

// Sample overloads below (would be generated per action)

/**
 * Check out a Git repository at a particular version.
 * @see https://github.com/actions/checkout
 */
export declare function uses(
  action: "actions/checkout",
  options?: UsesOptions<{
    /** The repository name with owner. Defaults to `${{ github.repository }}`. */
    repository?: string;
    /** The branch, tag or SHA to checkout. Defaults to the ref that triggered the workflow. */
    ref?: string;
    /** Personal access token used to fetch the repository. */
    token?: string;
